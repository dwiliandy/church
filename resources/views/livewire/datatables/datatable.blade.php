                    ...
                    <div class="flex rounded-lg w-96 shadow-sm">
                        <div class="relative flex-grow focus-within:z-10">
                            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                <svg class="w-5 h-5 text-green-400" viewBox="0 0 20 20" stroke="currentColor" fill="none">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                                </svg>
                            </div>
                            <input wire:model.debounce.500ms="search" style="padding-right: 35px" class=" search block w-full leading-9 pl-10 font-medium tracking-wider  text-xs space-x-2 border-gray-300 rounded-md" placeholder="{{__('Cari di')}} {{ $this->searchableColumns()->map->label->join(', ') }}" type="text" />
                            <div class="absolute inset-y-0 right-0 flex items-center pr-2">
                                <button wire:click="$set('search', null)" class="text-red-300 hover:text-red-600 focus:outline-none">
                ...
                <div class="table min-w-full align-middle ">
                    ...
                    <div class="table-row bg-blue-100 divide-x divide-blue-200">
                        @foreach($this->columns as $index => $column)
                            @if($column['hidden'])
                                @if($hideable === 'inline')
                                <div class="table-cell table-body w-5 overflow-hidden align-top bg-green-500"></div>
                                @endif
                            @elseif($column['type'] === 'checkbox')
                ...
                <div class="items-center justify-between p-2 sm:flex">
                    ...
                    @if(count($this->results))
                        <div class="flex items-center my-2 sm:my-0">
                            <select name="perPage" class="page-show block w-full py-2 pl-3 pr-10 mt-1 text-base border-gray-300 form-select leading-6 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5" wire:model="perPage">
