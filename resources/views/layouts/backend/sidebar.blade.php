<ul class="navbar-nav sidebar sidebar-dark accordion" id="accordionSidebar" style="background: #1e293b;">
	...
	<!-- Sidebar - Brand -->
	<a id="web_logo" class="sidebar-brand d-flex align-items-center justify-content-center py-4"
		href="{{ route('admin_dashboard') }}">
		<div class="sidebar-brand-icon">
			<img src="{{ asset('storage/' . DB::table('web_settings')->first()->logo) }}" alt=""
				style="max-height: 45px; border-radius: 8px;">
		</div>
		<div class="sidebar-brand-text mx-3" style="font-weight: 700; letter-spacing: 0.5px;">
			{{ DB::table('web_settings')->first()->website_name }}</div>
	</a>
	...
	<div class="sidebar-heading mt-3 mb-1" style="color: rgba(255,255,255,0.4); font-size: 0.7rem; letter-spacing: 1px;">
		Master Data
	</div>
	<!-- Nav Item - Dashboard -->
	...
	<li class="nav-item {{ Request::routeIs('approvals.index') ? 'active' : '' }}">
		<a class="nav-link" href="{{ route('approvals.index') }}">
			...
			<span>Persetujuan </span><span
				class="badge badge-danger">{{ DB::table('approvals')->where('approver', Auth::user()->id)->where('status', 'Menunggu')->count() }}</span>
		</a>
	</li>
